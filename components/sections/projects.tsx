...
export default function Projects() {
  ...
  useEffect(() => {
    if (isHovering && swiperRef.current) swiperRef.current.autoplay.pause()
    else if (swiperRef.current) swiperRef.current.autoplay.resume()
  }, [isHovering])
    ...
    <section id="projects" className="py-20 bg-black border-t border-gray-800/20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }}>
          ...
          <motion.p className="text-gray-400 text-lg mb-8" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: 0.1, duration: 0.5 }} viewport={{ once: true }}>
            <WordRevealAnimation
          ...
          <div className="flex justify-between items-center mb-6">
            <div className="flex gap-2">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => swiperRef.current?.slidePrev()} className="p-3 rounded-full bg-gray-900/80 backdrop-blur-sm border border-gray-700/50 text-white hover:border-gray-500 transition-all duration-200">
                <ChevronLeft size={20} />
              </motion.button>
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => swiperRef.current?.slideNext()} className="p-3 rounded-full bg-gray-900/80 backdrop-blur-sm border border-gray-700/50 text-white hover:border-gray-500 transition-all duration-200">
          ...
          {/* Swiper Slider */}
          <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
            <Swiper
              ...
              slidesPerView={1}
              speed={600}
              breakpoints={{ 640: { slidesPerView: 1, spaceBetween: 20 }, 768: { slidesPerView: 2, spaceBetween: 20 }, 1024: { slidesPerView: 3, spaceBetween: 25 } }}
              pagination={{ clickable: true, dynamicBullets: true, renderBullet: (index, className) => `<span class="${className} bg-gray-600 hover:bg-gray-400 transition-colors duration-200"></span>` }}
            ...
            >
              {projects.map((project) => (
                <SwiperSlide key={project.title}>
                  <motion.div variants={cardVariants} whileHover={{ y: -8 }} className="group bg-gradient-to-br from-gray-900/40 to-black border border-gray-700/30 rounded-xl overflow-hidden hover:border-gray-500/50 transition-all duration-300 flex flex-col h-full relative backdrop-blur-sm">
                    ...
                    <div className="p-5 flex flex-col flex-grow">
                      <h3 className="text-lg font-semibold text-white mb-2 group-hover:text-gray-300 transition-colors duration-300 line-clamp-1">{project.title}</h3>
                      <p className="text-gray-400 text-sm mb-4 flex-grow leading-relaxed line-clamp-3">{project.description}</p>

                      <div className="flex flex-wrap gap-1.5 mb-4">
                        {project.tech.slice(0, 3).map((t) => (
                          <span key={t} className="px-2 py-1 bg-gray-900/50 border border-gray-700/50 text-gray-400 rounded text-xs font-medium transition-colors hover:border-gray-500">{t}</span>
                        ))}
                        {project.tech.length > 3 && (
                          <span className="px-2 py-1 bg-gray-900/50 border border-gray-700/50 text-gray-400 rounded text-xs font-medium">+{project.tech.length - 3}</span>
                        )}
                      </div>
                      ...
                      {project.status === "live" ? (
                        ...
                        >
                          <div className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-yellow-600/20 to-orange-600/20 border border-yellow-500/30 text-yellow-400 w-full">
                            <Rocket size={16} />
